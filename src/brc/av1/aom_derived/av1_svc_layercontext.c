...
void av1_init_layer_context(AV1_COMP *const cpi) {
  ...
  for (int sl = 0; sl < svc->number_spatial_layers; ++sl) {
    for (int tl = 0; tl < svc->number_temporal_layers; ++tl) {
      ...
      AV1_LAYER_CONTEXT *const lc = &svc->layer_context[layer];
      AV1_RATE_CONTROL *const lrc = &lc->rc;
      ...
      for (int i = 0; i < AV1_RATE_FACTOR_LEVELS; ++i) {
        lrc->rate_correction_factors[i] = 1.0;
      }
      lc->target_bandwidth = lc->layer_target_bitrate;
      lrc->last_q[AV1_INTER_FRAME] = lrc->worst_quality;
      lrc->avg_frame_qindex[AV1_INTER_FRAME] = lrc->worst_quality;
      lrc->avg_frame_qindex[AV1_KEY_FRAME] = lrc->worst_quality;
      lrc->buffer_level =
 ...
 *
 * \return AV1_LAYER_CONTEXT for current layer.
 */
static AV1_LAYER_CONTEXT *get_layer_context(AV1_COMP *const cpi) {
...
void av1_update_temporal_layer_framerate(AV1_COMP *const cpi) {
  AV1_SVC *const svc = &cpi->svc;
  AV1_LAYER_CONTEXT *const lc = get_layer_context(cpi);
  AV1_RATE_CONTROL *const lrc = &lc->rc;
  ...
  if (tl == 0) {
    ...
    AV1_LAYER_CONTEXT *const lcprev = &svc->layer_context[prev_layer];
...
void av1_restore_layer_context(AV1_COMP *const cpi) {
  ...
  // ToDo: For spatial-svc, allow cyclic-refresh to be applied on the spatial layers,
  ...
  if (svc->external_ref_frame_config && svc->force_zero_mode_spatial_ref) {
    int ref_frame_idx = svc->ref_idx[AV1_LAST_FRAME - 1];
    ...
    ref_frame_idx = svc->ref_idx[AV1_GOLDEN_FRAME - 1];
    if (svc->buffer_time_index[ref_frame_idx] == svc->current_superframe &&
        svc->buffer_spatial_layer[ref_frame_idx] <= svc->spatial_layer_id - 1)
...
void av1_save_layer_context(AV1_COMP *const cpi) {
  AV1_SVC *const svc = &cpi->svc;
  AV1_LAYER_CONTEXT *lc = get_layer_context(cpi);
  lc->rc = cpi->rc;
  ...
  if (cpi->common.current_frame.frame_type == AV1_KEY_FRAME) {
    ...
    for (unsigned int i = 0; i < AV1_REF_FRAMES; i++) {
    ...
    for (unsigned int i = 0; i < AV1_INTER_REFS_PER_FRAME; i++) {

...
#define AV1_MAX_STATIC_GF_GROUP_LENGTH 250

#define AV1_MIN_GF_INTERVAL 4
#define AV1_MAX_GF_INTERVAL 32
#define AV1_FIXED_GF_INTERVAL 16
#define AV1_MAX_GF_LENGTH_LAP 16

#define AV1_MINQ 0
#define AV1_MAXQ 255
#define AV1_QINDEX_RANGE (AV1_MAXQ - AV1_MINQ + 1)

// Total number of QM sets stored
#define AV1_QM_LEVEL_BITS 4
#define AV1_NUM_QM_LEVELS (1 << AV1_QM_LEVEL_BITS)
/* Range of QMS is between first and last value, with offset applied to inter
 * blocks*/
#define AV1_DEFAULT_QM_Y 10
#define AV1_DEFAULT_QM_U 11
#define AV1_DEFAULT_QM_V 12
#define AV1_DEFAULT_QM_FIRST 5
#define AV1_DEFAULT_QM_LAST 9

#define AV1_MAX_NUM_GF_INTERVALS 15

#define AV1_MAX_ARF_LAYERS 6
// #define STRICT_RC
...
typedef enum {
  STATUS_BRC_AV1_SUCCESS,
  STATUS_BRC_AV1_FAILED,
  STATUS_BRC_AV1_UNSUPPORTED_SPATIAL_SVC,
  STATUS_BRC_AV1_UNSUPPORTED_TEMPORAL_SVC,
  STATUS_BRC_AV1_INVALID_PARAM,
...
typedef enum {
  ...
  AV1_VARIANCE_AQ = 1,
  AV1_COMPLEXITY_AQ = 2,
  AV1_CYCLIC_REFRESH_AQ = 3,
  AV1_AQ_MODE_COUNT  // This should always be the last member of the enum
} AV1_AQ_MODE;

typedef enum {
  AV1_NO_DELTA_Q = 0,
  AV1_DELTA_Q_OBJECTIVE = 1,   // Modulation to improve objective quality
  AV1_DELTA_Q_PERCEPTUAL = 2,  // Modulation to improve perceptual quality
  AV1_DELTA_Q_MODE_COUNT       // This should always be the last member of the enum
} AV1_DELTAQ_MODE;
...
typedef enum OVERSHOOT_DETECTION_CBR {
 ...
 */
typedef struct AV1_SPEED_FEATURES {
 /*!
...
typedef enum {
  ...
  AV1_GF_ARF_STD,
  AV1_KF_STD,
  AV1_RATE_FACTOR_LEVELS
} AV1_RATE_FACTOR_LEVEL;
...
typedef enum {
  AV1_NO_RESIZE = 0,
  AV1_DOWN_THREEFOUR = 1,  // From orig to 3/4.
  AV1_DOWN_ONEHALF = 2,    // From orig or 3/4 to 1/2.
  AV1_UP_THREEFOUR = -1,   // From 1/2 to 3/4.
  AV1_UP_ORIG = -2,        // From 1/2 or 3/4 to orig.
} AV1_RESIZE_ACTION;

typedef enum { AV1_ORIG = 0, AV1_THREE_QUARTER = 1, AV1_ONE_HALF = 2 } AV1_RESIZE_STATE;
 ...
 */
typedef struct CommonQuantParams {
  /*!
   ...
   */
  const qm_val_t *giqmatrix[AV1_NUM_QM_LEVELS][3][TX_SIZES_ALL];
  /*!
   * Global quantization matrix table.
   */
  const qm_val_t *gqmatrix[AV1_NUM_QM_LEVELS][3][TX_SIZES_ALL];
  /**@}*/
 ...
 /**
   ...
   *  - If false, we implicitly use level index 'AV1_NUM_QM_LEVELS - 1'.
   */
...
enum {
 ...
 // Extra/scratch reference frame. It may be:
  ...
  AV1_EXTREF_FRAME = AV1_REF_FRAMES,

  // Number of inter (non-intra) reference types.
  AV1_INTER_REFS_PER_FRAME = AV1_ALTREF_FRAME - AV1_LAST_FRAME + 1,

  // Number of forward (aka past) reference types.
  AV1_FWD_REFS = AV1_GOLDEN_FRAME - AV1_LAST_FRAME + 1,

  // Number of backward (aka future) reference types.
  AV1_BWD_REFS = AV1_ALTREF_FRAME - AV1_BWDREF_FRAME + 1,

  AV1_SINGLE_REFS  = AV1_FWD_REFS + AV1_BWD_REFS,
};
...
typedef struct {
  ...
  FRAME_UPDATE_TYPE update_type[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  unsigned char arf_src_offset[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  // The number of frames displayed so far within the GOP at a given coding
  // frame.
  unsigned char cur_frame_idx[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  int layer_depth[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  int arf_boost[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  int max_layer_depth;
  int max_layer_depth_allowed;
  // This is currently only populated for AOM_Q mode
  unsigned char q_val[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  int bit_allocation[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  // The frame coding type - inter/intra frame
  AV1_FRAME_TYPE frame_type[AV1_MAX_STATIC_GF_GROUP_LENGTH];
  // The reference frame buffer control - update or reset
  REFBUF_STATE refbuf_state[AV1_MAX_STATIC_GF_GROUP_LENGTH];
...
typedef struct {
   ...
   * in the encode loop.
   */
  double rate_correction_factors[AV1_RATE_FACTOR_LEVELS];
   ...
   */
  int gf_intervals[AV1_MAX_NUM_GF_INTERVALS];
  ...
  int ni_tot_qi;
  int ni_frames;
  int avg_frame_qindex[AV1_FRAME_TYPES];
  double tot_q;
  double avg_q;
   ...
   */
  int active_best_quality[AV1_MAX_ARF_LAYERS + 1];
...
void av1_rc_init(const AV1EncoderConfig *oxcf, int pass,
                 AV1_RATE_CONTROL *rc);

int av1_estimate_bits_at_q(AV1_FRAME_TYPE frame_kind, int q, int mbs,
...
void av1_rc_compute_frame_size_bounds(const AV1_COMP *cpi,
...
int av1_rc_clamp_iframe_target_size(const AV1_COMP *const cpi,
                                    int target);
int av1_rc_clamp_pframe_target_size(const AV1_COMP *const cpi,
                                    int target, FRAME_UPDATE_TYPE frame_update_type);
